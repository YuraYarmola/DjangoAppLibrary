...
class Author(models.Model):
    ...
    def create(name, surname, patronymic):
        ...
        if name and len(name) <= 20 and surname and len(surname) <= 20 and patronymic and len(patronymic) <= 20:
            id = Author.objects.all().order_by("-id")[0].id
            author = Author(id=id+1, name=name, surname=surname, patronymic=patronymic)
            author.save()
            return author
    ...
    def get_absolute_url(self):
        return reverse('author_info', kwargs={'author_id': str(self.id)})
